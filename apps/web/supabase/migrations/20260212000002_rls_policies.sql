...
-- ============================================================================
-- PART 1: HELPER FUNCTIONS IN AUTH SCHEMA
-- ============================================================================
...
COMMENT ON FUNCTION auth.get_user_org_id IS 'Returns organization_id for the current authenticated user. STABLE for per-transaction caching.';
...
COMMENT ON FUNCTION auth.is_org_admin IS 'Returns true if user has admin/management role (super_admin, gerente_general, director_comercial, gerente_operativo)';
...
COMMENT ON FUNCTION auth.is_commercial_manager IS 'Returns true if user has commercial manager role or higher';
...
COMMENT ON FUNCTION auth.has_perm IS 'Returns true if user has a specific permission (module:action format)';
  ...
  USING (
    EXISTS (
      ...
      WHERE r.id = role_permissions.role_id
        AND r.organization_id = auth.get_user_org_id()
    )
  );
  ...
  USING (
    EXISTS (
      ...
      WHERE p.id = user_roles.user_id
        AND p.organization_id = auth.get_user_org_id()
    )
  );
...
CREATE POLICY "leads_select"
  ...
  USING (
    organization_id = auth.get_user_org_id()
    AND (
      auth.is_commercial_manager()  -- Managers see all
...
CREATE POLICY "quotes_select"
  ...
  USING (
    ...
    AND (
      ...
      OR auth.has_perm('quotes:read') -- Finanzas/other roles can see all
    )
  );
  ...
  USING (
    EXISTS (
      ...
      WHERE o.id = order_items.order_id
        AND o.organization_id = auth.get_user_org_id()
    )
  );
  ...
  USING (
    EXISTS (
      ...
      WHERE po.id = purchase_order_items.purchase_order_id
        AND po.organization_id = auth.get_user_org_id()
    )
  );
  ...
  USING (
    EXISTS (
      ...
      WHERE s.id = shipment_items.shipment_id
        AND s.organization_id = auth.get_user_org_id()
    )
  );
  ...
  USING (
    EXISTS (
      ...
      WHERE i.id = invoice_items.invoice_id
        AND i.organization_id = auth.get_user_org_id()
    )
  );
...
-- Index on profiles for auth.get_user_org_id() (already exists as PK)
...
HELPER FUNCTIONS:
1. auth.get_user_org_id() - Returns org_id for current user (STABLE)
2. auth.is_org_admin() - Check if user has admin/management role
3. auth.is_commercial_manager() - Check if user has commercial manager role+
4. auth.has_perm(slug) - Check specific permission
