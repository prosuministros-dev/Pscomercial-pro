-- Fix BUG-015: profiles table uses full_name, not display_name
-- Affects: get_order_traceability, get_users_by_role

-- 1. Fix get_order_traceability
...
BEGIN
  ...
  LOOP
    v_events := v_events || jsonb_build_object(
      'timestamp', v_row.ts, 'type', v_row.event_type,
      'title', v_row.title, 'description', v_row.description,
      'user_name', v_row.user_name, 'metadata', v_row.meta
    );
  END LOOP;
...
BEGIN
  ...
  SELECT
    p.id AS user_id,
    p.email::text,
    COALESCE(p.full_name, p.email)::text AS display_name
  FROM user_roles ur
