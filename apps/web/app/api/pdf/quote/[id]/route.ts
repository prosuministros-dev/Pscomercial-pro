 ...
 * Generate PDF for a quote, upload to storage, return signed URL
...
export async function GET(
  ...
  try {
    ...
    // Fetch quote with all needed data
    ...
    const pdfBuffer = await renderToBuffer(
      QuotePdfTemplate({ quote, org }) as React.ReactElement,
    );

    const fileName = `cotizacion-${quoteData.quote_number}.pdf`;
    const storagePath = `${quoteData.organization_id}/quotes/${fileName}`;
